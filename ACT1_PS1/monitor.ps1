...
function Test-RamUsage {
    ...
    if ($uso -gt $UmbralRam) {
        ...
        Write-MonitorLog "⚠️ RAM supera el $UmbralRam%. Top procesos por memoria:"
...
function Test-CpuUsage {
    ...
    Write-MonitorLog "⚙️ CPU: uso $uso%"
    ...
    if ($uso -gt $UmbralCpu) {
        ...
        Write-MonitorLog "⚠️ CPU supera el $UmbralCpu%. Top procesos por tiempo de CPU:"
        $procesos -split "`r?`n" | Where-Object { $_ } | ForEach-Object {
            Write-MonitorLog "   $_"
        }
...
if (Import-EnvFile) {
    ...
    Write-MonitorLog "⚠️ No se encontró $EnvFile - las alertas de Telegram quedarán desactivadas."
}
